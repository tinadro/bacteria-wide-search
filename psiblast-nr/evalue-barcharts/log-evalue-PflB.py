...
ls = pd.read_csv(fpath, sep='\t')

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# CONVERT DATA TO LOG VALUES:
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

zero = [i for i in ls['evalue'] if i == 0] # list of all the zeroes 
data = [i for i in ls['evalue'] if i != 0] # list of all non-zero evalues
...
plt.margins(x=.005)
plt.tight_layout()
plt.savefig('PflB-logevalue', dpi=300)
plt.show()
