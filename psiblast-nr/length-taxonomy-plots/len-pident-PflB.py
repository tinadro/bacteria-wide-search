...
fpath = parentdir+'/psiblast-nr-PflB-taxonomy.tsv'
...
for ind, row in f.iterrows():
	if row['b'] == 'Bacteria':
		...
		y_bac.append(row['pident'])
	elif row['b'] == 'Archaea':
		x_ar.append(row['slen'])
		y_ar.append(row['pident'])
	elif row['b'] == 'Eukaryota':
		x_eu.append(row['slen'])
		y_eu.append(row['pident'])
...
ax.scatter(x_bac, y_bac, color='#f70027', marker='.', alpha=0.33, edgecolor='k', linewidth=0.4, label='Bacteria')
ax.scatter(x_ar, y_ar, color='#7cfc00', marker='.', alpha=0.33, edgecolor='k', linewidth=0.4, label='Archaea')
ax.scatter(x_eu, y_eu, color='#00bfff', marker='.', alpha=0.33, edgecolor='k', linewidth=0.4, label='Eukaryotes')
...
plt.legend()
plt.tight_layout()
plt.savefig('PflB-len-pident-tax', dpi=300)
plt.show()
